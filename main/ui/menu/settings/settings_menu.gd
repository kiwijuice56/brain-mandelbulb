...
func _ready() -> void:
	for node in get_tree().get_nodes_in_group("Option"):
		...
		if node is LineEdit:
			node.text_changed.connect(_on_option_selected.unbind(1))
	...
	%MuseDeviceNameLineEdit.text = GlobalData.settings_configuration.muse_device_name
...
func _on_option_selected() -> void:
	...
	GlobalData.settings_configuration.muse_device_name = %MuseDeviceNameLineEdit.text
